...
- 「意味を見る」ボタン（右固定）
- keywords.csvから修練/上級レベルを「原形＋意味」の組み合わせでユニーク化し、日付ハッシュで日替わり選択
- 熟語記法ありの場合、カードにはdisplayWord（熟語形）を表示
- モーダル: 選ばれたレコードの品詞・意味のみ表示。出現大問も同一意味のレコードに限定。単語帳掲載状況は原形で照合し、見出しに `"原形" の単語帳掲載状況` と表示
...
| 2026年3月 | **4.1** | 「今日の難単語」の選定単位を原形ベースからレコード単位（原形＋意味）に変更。熟語記法レコードが選ばれた場合に熟語形を表示し、モーダルの品詞・意味・出現大問も該当レコードに限定 |
