...
3. **今日の難単語カード**
   ...
   - 単語名を中央揃えで大きく表示（text-2xl font-bold）
   - 「意味を見る」ボタン（bg-emerald-600、中央揃え）でモーダル表示
   - アイコン: Sparkles（Lucide React）、amber系カラー

4. **今日の難単語モーダル**
   - ヘッダー: 単語名（text-2xl font-bold）+ 閉じるボタン（X）
   - **品詞・意味**（目立つカード風）:
     - 品詞ごとにグルーピング（getWordInfoGroupedを使用）
     - 各品詞をemeraldバッジ（bg-emerald-600 白文字）で表示
     - 意味はtext-baseで大きめに表示
     - **熟語記法対応**: 熟語形をイタリック小フォントで併記、意味は`<<>>`を除去して表示
     - 背景色付きカード（bg-gray-50 + border）
   - **出現した大問**:
     - keywords.csvから該当単語の大問IDを収集（ユニーク化）
     - 各行: 年度 大学名 学部（/mondai/[識別名]へのリンク）
   - **単語帳の掲載状況**（シンプルリスト形式）:
     - 各行: ◯/△ + 単語帳名（Amazonリンク付き）+ No./p.
   - フッター: 「閉じる」ボタン（bg-emerald-600）

5. **最新記事セクション**（準備中プレースホルダー）
...
- ボタン式チェックボックス
- 各レベルに「すべて選択/解除」ボタン
- **熟語記法対応**: 意味に`<<熟語形>>`を含む場合、原形の代わりに熟語形を単語名として表示。意味は`<<>>`を除去して表示
- 機能ボタン:
  - ✓の単語をコピー
  - ✓の単語を単語帳比較へ（新しいタブ）
...
1. **レベル選択画面**:
   ...
   - レベル別語数は品詞フィルタに連動
   - **「この大問で出題された意味のみをチェック」トグル**: デフォルトOFF、ピンク色（bg-pink-300）
     - ON: keyword由来の意味のみ隠し、それ以外の意味は最初から表示
     - OFF: 従来通りすべての意味を非表示
   - 凡例（`<span class="bg-pink-100">　</span> この大問で出現した意味`）は「一覧」「チェック」ボタンの下に配置
2. **単語チェック中**:
   - プログレスバー
   - 単語を大きく表示
   - **熟語記法対応**: 熟語形がある場合は熟語形をメイン表示、原形は小さく薄く表示。意味は熟語形の意味のみ表示（原形の他の意味は非表示）
   - 熟語形がない場合: 従来通り品詞+レベルタグ、意味表示（keyword由来はピンクハイライト）
   - 「自信なし」「自信あり」ボタン
   - ランダム順で出題
   - ブロックに最低高さ（min-h-[420px]）を設定し、ボタン位置を安定化
3. **結果画面**:
   - 2カラム（自信なし/自信あり）
   - 各単語の下に品詞・意味を表示
   - **熟語記法対応**: 熟語形がある場合は原形の代わりに熟語形を表示
   - 複数レベルにまたがる単語は全レベルの絵文字を表示
   - チェックボックスで選択
   - コピー・単語帳比較へ連携

7. **単語帳掲載状況セクション**
   - 見出し（📖 単語帳掲載状況）の1段下に単語帳選択プルダウンを中央配置

#### 単語詳細モーダル
- ヘッダーに品詞・意味・レベルを表示
  - 品詞ごとにレベル名（テキスト）を左側に表示
  - keyword由来の意味はピンク太字（text-pink-400）でハイライト
  - **熟語記法対応**: 熟語形をイタリックで併記、意味は`<<>>`を除去して表示
- 各単語帳での掲載状況一覧
...
- **熟語記法対応**: 熟語形をイタリックで表示、意味は`<<>>`を除去して表示
...
- **意味フィルタ**: keywords.csvに存在する意味をトグルボタンとして表示（複数選択可、デフォルト全選択）。選択した意味で大問をフィルタリング。熟語記法は「熟語形: 意味」の形式で表示
...
**グルーピングトグル（2段階）**:
- 検索結果ヘッダーに2つのトグルスイッチを配置
- トグル1「同じ品詞・異なる意味をまとめる」（デフォルトON）: 同一原形+品詞のレコードを1行に集約
- トグル2「同じ綴り・異なる品詞もまとめる」（デフォルトON）: 品詞を跨いで1行に集約
- トグル2 ONならトグル1も強制ON（トグル1はemerald-400で無効表示）
- 有効な組み合わせ: 両方OFF / トグル1のみON / 両方ON
- 切替時に検索結果を再計算（useEffectで連動）
...
- 行クリックで単語詳細モーダル表示
- グルーピングON時: 品詞に`+N`（他の品詞数）、意味に`+N`（他の意味数）を表示
- 熟語記法を含む場合: 単語欄に「※熟語形」をカッコ書きで併記（小さめグレー）、意味欄にも※印を付与
...
**単語詳細モーダル（レスポンシブ対応）**:
- 単語クリックでモーダル表示（max-w-2xl、スマホではほぼ画面幅）
- ヘッダー: 単語名 + 品詞バッジ（+N表示対応）+ 出題回数
- 大学フィルタトグル（検索時にフィルタが適用されている場合のみ表示）:
  - 「絞込中の大学のみ」（デフォルト）/「すべての検索結果」の2択
  - 切替時にselectedMeaningsをリセットし全選択
  - 「すべての検索結果」ではkeywords.csv全体から該当単語の全品詞・全意味・全大問を表示
- 意味一覧エリア: 品詞ごとにグルーピング表示
  - 品詞が複数ある場合のみ品詞ラベル表示
  - 各意味はトグルボタン（emerald-600 / gray-100）、出題回数を小さく併記
  - 熟語記法はparseIdiomNotationで表示（displayWord + displayMeaning）
  - 意味の選択/解除にリアルタイムで大問一覧が連動
- 大問一覧:
  - PC: テーブル（大学名/年度/日程/学部/大問番号/ジャンル+ハッシュタグ）
  - スマホ: カード形式（大学名+年度、日程·学部·大問番号、ジャンル+ハッシュタグ）
  - selectedMeaningsに該当する大問のみ表示（フィルタ中は件数表示）
  - 大問クリックで新規タブに大問詳細ページを開く
- フッター: 注意書き「※出題回数の多すぎる意味は掲載していません」+ 閉じるボタン
...
- `isIdiom`: `<<>>`が含まれていたかのboolean

**適用箇所**:
- 大問詳細: 一覧モード、チェックモード（出題・結果）、単語詳細モーダル
- トップページ: 今日の難単語モーダル
- 単語検索: 検索結果ヘッダー、意味フィルタボタン

**注意**: 原形（preventなど）はそのまま維持。検索・単語帳照合・URLパラメータ・コピー機能はすべて原形を使用。

### 10.6 サーバー/クライアントコンポーネント分離
...
| 2026年2月 | **3.4** | **トップページ「今日の難単語」追加**: 旧「記事（準備中）」カードを日替わり難単語カードに変更、モーダルで品詞・意味（カード風）・出現大問・単語帳掲載状況を表示 |
| 2026年2月 | **3.4.1** | **熟語記法（<<>>記法）対応**: `parseIdiomNotation`ユーティリティ追加、大問詳細（一覧/チェック/結果/モーダル）・トップページ（今日の難単語）・単語検索（検索結果/意味フィルタ）で熟語表示に対応。チェックモードの「出題意味のみ」トグルのデフォルトをOFFに変更。チェックモードに最低高さ設定（ボタン位置安定化）。凡例を「一覧」「チェック」ボタンの下に移動。単語帳掲載状況のプルダウンを中央配置に変更 |
| 2026年2月 | **3.5** | **大学別検索モードの大幅改善**: 2段階グルーピングトグル追加（意味まとめ/品詞またぎまとめ）、テーブルに品詞+N・意味+N・熟語記法カッコ書き（※印）表示、単語詳細モーダル刷新（大学フィルタトグル「絞込中の大学のみ/すべての検索結果」、意味の品詞別グルーピング表示、意味トグルによる大問リアルタイムフィルタ、スマホ用カード形式レスポンシブ対応、大問クリックで新規タブ遷移、注意書き追加） |