...
- **今日の難単語**: トップページで日替わりの難単語を表示。keywords.csvから修練・上級レベルを「原形＋意味」の組み合わせでユニーク化し、日付ハッシュで1つ選択。熟語記法ありの場合はdisplayWord（熟語形）をカードに表示。モーダルでは選ばれたレコードの品詞・意味のみ表示し、出現大問も同一意味のレコードに限定。単語帳照合は原形で実施し、見出しに `"原形" の単語帳掲載状況` と表示。最大幅450px、中央揃え
