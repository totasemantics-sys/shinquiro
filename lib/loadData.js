...
export function getUniversityCodeFromId(識別名, universities) {
  ...
  const validUniversities = universities.filter(u => u.大学コード && u.大学コード.length > 0);
  const sortedUniversities = [...validUniversities].sort((a, b) => 
    b.大学コード.length - a.大学コード.length
  );
  
  for (const uni of sortedUniversities) {
    if (識別名.startsWith(uni.大学コード)) {
      return uni.大学コード;
    }
  }
...
export function getUniversityName(code, universities) {
  ...
  const uni = universities.find(u => u.大学コード === code);
  return uni?.大学名 || '';
}
...
export function getUniversityCode(name, universities) {
  ...
  const uni = universities.find(u => u.大学名 === name);
  return uni?.大学コード || '';
}